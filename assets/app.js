// Laster data/news.json uansett om du kjører lokalt eller på GitHub Pages
...
async function loadData() {
  ...
  if (!json?.items || !Array.isArray(json.items)) {
    throw new Error("Ugyldig dataformat: forventer { items: [] }");
  }
...
function renderItems(items) {
  ...
  if (!items.length) {
    list.innerHTML = `
      <div class="card">
        <div class="top">Ingen treff</div>
        <h3>Fant ingen saker</h3>
      </div>`;
    return;
  }
  ...
  for (const item of items) {
    ...
    const safeTitle = item.title ?? "Uten tittel";
